    ...
    [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([PHPhotoLibrary authorizationStatus] != PHAuthorizationStatusAuthorized) {
                ...
                UIAlertController *alert = [UIAlertController alertControllerWithTitle:[NSBundle hx_localizedStringForKey:@"无法访问相册"] message:[NSBundle hx_localizedStringForKey:@"请前往设置-隐私-相册中允许访问相册"] preferredStyle:UIAlertControllerStyleAlert];
                ...
                [alert addAction:[UIAlertAction actionWithTitle:[NSBundle hx_localizedStringForKey:@"去设置"] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (void)cancelClick {
    ...
    NSLog(@"cancelBlock:%@",self.cancelBlock);
    if (self.cancelBlock) {
...
//HXDatePhotoViewController 点击右上角“取消”按钮时执行的代理方法
...
- (void)getAlbumModelList:(BOOL)isFirst {
    ...
    if (self.manager.albums.count > 0 && self.manager.configuration.saveSystemAblum && !self.manager.configuration.singleSelected) {
        ...
        vc.title = [HXPhotoTools transFormPhotoTitle:model.albumName];
        
        vc.UploadUrl = self.UploadUrl;
        vc.MsgBody = self.MsgBody;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    NSLog(@"%zd",self.albumModelArray.count);
    return self.albumModelArray.count;
}
...
- (UIViewController *)previewingContext:(id<UIViewControllerPreviewing>)previewingContext viewControllerForLocation:(CGPoint)location {
    ...
    vc.callBack = self.callBack;
    vc.manager = self.manager;
    vc.title = [HXPhotoTools transFormPhotoTitle:cell.model.albumName];
...
- (UILabel *)authorizationLb {
    if (!_authorizationLb) {
        ...
        _authorizationLb.text = [NSBundle hx_localizedStringForKey:@"无法访问相册\n请前往设置中允许应用访问相册"];
...
- (void)setModel:(HXAlbumModel *)model {
    ...
    self.albumNameLb.text = [HXPhotoTools transFormPhotoTitle:model.albumName];
